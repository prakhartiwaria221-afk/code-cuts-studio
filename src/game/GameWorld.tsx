...
const GameWorld = () => {
  ...
  const update = () => {
    ...
    // Check nearby interactables for hint
    const cx = Math.floor((p.x + TILE_SIZE / 2) / TILE_SIZE);
    const cy = Math.floor((p.y + TILE_SIZE / 2) / TILE_SIZE);
    let foundHint = '';
  ...
  const render = (ctx: CanvasRenderingContext2D, cw: number, ch: number) => {
    ...
    BUILDING_LABELS.forEach(label => {
      const lx = label.x * TILE_SIZE - cam.x;
      const ly = label.y * TILE_SIZE - cam.y;
  ...
  const drawTile = (ctx: CanvasRenderingContext2D, tile: number, x: number, y: number, tx: number, ty: number) => {
    ...
    switch (tile) {
      ...
      case T.WATER: {
        ...
        ctx.fillRect(x + off, y + 8, 8, 2);
        ctx.fillRect(x + (off + 16) % s, y + 22, 6, 2);
        break;
      }
      ...
      case T.ROOF:
        ...
        for (let i = 0; i < s; i += 8) {
          ctx.fillRect(x + i, y + ((i / 8) % 2) * 4, 8, 4);
        }
        break;

      case T.DOOR_ABOUT:
      case T.DOOR_SKILLS:
      case T.DOOR_PROJECTS:
      case T.DOOR_CONTACT:
        ctx.fillStyle = '#7a6b5a';
        ctx.fillRect(x, y, s, s);
        // Door frame
      ...
      case T.LAMP:
        ...
        ctx.beginPath();
        ctx.arc(x + 16, y + 7, 14, 0, Math.PI * 2);
        ctx.fill();
        break;
  ...
  const drawPlayer = (ctx: CanvasRenderingContext2D, x: number, y: number, dir: Direction, frame: number) => {
    ...
    if (dir === 0) { // Down
      ...
      ctx.fillRect(x + 5 * s, y + 1 * s, 6 * s, 6 * s);
      ctx.fillStyle = '#3d2b1f';
      ctx.fillRect(x + 5 * s, y + 0 * s, 7 * s, 2 * s);
      ctx.fillRect(x + 11 * s, y + 1 * s, 1 * s, 3 * s);
      ctx.fillStyle = '#222';
      ctx.fillRect(x + 6 * s, y + 4 * s, 2 * s, 1 * s);
    } else { // Right
      ctx.fillRect(x + 5 * s, y + 1 * s, 6 * s, 6 * s);
      ctx.fillStyle = '#3d2b1f';
      ctx.fillRect(x + 4 * s, y + 0 * s, 7 * s, 2 * s);
      ctx.fillRect(x + 4 * s, y + 1 * s, 1 * s, 3 * s);
      ctx.fillStyle = '#222';
      ctx.fillRect(x + 9 * s, y + 4 * s, 2 * s, 1 * s);
    }
    ...
    return (
      <div className="fixed inset-0 bg-[#1a1a2e] flex flex-col items-center justify-center z-50" style={{ fontFamily: '"Press Start 2P", monospace' }}>
        ...
        <div className="text-center mb-12 animate-fade-in">
          <h1 className="text-[#4a8c3f] text-3xl sm:text-5xl mb-4 leading-relaxed tracking-wider">
            PRAKHAR
          </h1>
          <h1 className="text-[#5aafc8] text-3xl sm:text-5xl mb-6 leading-relaxed tracking-wider">
            TIWARI
          </h1>
        ...
        <div className="absolute bottom-6 left-0 right-0 flex items-center justify-center gap-4">
          ...
          <a
            ...
            className="bg-[#0077b5] text-white px-4 py-1.5 text-[8px] hover:bg-[#005f8d] transition-colors"
          >
            LinkedIn
          </a>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-[#1a1a2e]" style={{ fontFamily: '"Press Start 2P", monospace' }}>
      ...
      <div className="absolute top-4 right-4 bg-black/70 px-4 py-2 border border-[#5aafc8]/50 hidden sm:block">
        <p className="text-[#5aafc8] text-[7px]">WASD Move | SPACE Interact</p>
      </div>
      ...
      {isMobile && (
        <>
          ...
          {/* Action button */}
          <button
            onTouchStart={() => { keysRef.current.add(' '); tryInteract(); }}
            onTouchEnd={() => keysRef.current.delete(' ')}
            className="absolute bottom-12 right-12 w-16 h-16 rounded-full bg-[#e74c3c]/80 border-2 border-[#e74c3c] text-white text-sm flex items-center justify-center active:scale-90 select-none"
          >
            A
          </button>
        </>
      ...
      {/* Dialog */}
      {gameState === 'dialog' && dialogKey && (
        <GameDialog
          content={PORTFOLIO_CONTENT[dialogKey as keyof typeof PORTFOLIO_CONTENT]}
          onClose={closeDialog}
        />
      )}
    </div>
